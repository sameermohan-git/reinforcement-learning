...
class MazeEnvMatplot(gym.Env):
    ...
    def __init__(self, maze, maze_size, start_cell=(0, 0), exit_cell=None):
    ...
    def __execute(self, action):
        ...
        if not possible_actions:
            ...
            if self.__current_cell == self.__exit_cell:
                reward = MazeEnvMatplot.reward_exit  # maximum reward when reaching the exit cell
            elif self.__current_cell in self.__visited:
                reward = MazeEnvMatplot.penalty_visited  # penalty when returning to a cell which was visited earlier
            else:
                reward = MazeEnvMatplot.penalty_move  # penalty for a move which did not result in finding the exit cell

            self.__visited.add(self.__current_cell)
        else:
            reward = MazeEnvMatplot.penalty_impossible_move  # penalty for trying to enter an occupied cell or move out of the maze
    ...
    def __possible_actions(self, cell=None):
        ...
        possible_actions = MazeEnvMatplot.actions.copy()  # initially allow all
    ...
    def __observe(self):
        ...
        """
        print(np.array([[*self.__current_cell]]))
